...
class CaptionGenerator(BaseModel):
    ...
    def build_gru(self):
        ...
        gru = tf.nn.rnn_cell.GRUCell(
            config.num_gru_units,
            kernel_initializer = self.nn.fc_kernel_initializer,
        )
        if self.is_train:          #训练的时候应用dropout,预测的时候不用
            gru  = tf.nn.rnn_cell.DropoutWrapper(
                gru,
                input_keep_prob=1.0-config.gru_drop_rate,
                output_keep_prob=1.0-config.gru_drop_rate
            )

        #初始化GRU
        with tf.variable_scope("initailize"):
            context_mean = tf.reduce_mean(self.conv_feats, axis = 1)
            initial_memory, initial_output  = self.initialize2(context_mean)
        ...
        for idx in range(num_steps):
            ...
            with tf.variable_scope("gru"):
                ...
                output, state = gru(current_input, last_state)
                memory= state

            # Decode the expanded output of GRU into a word
    ...
    def initialize2(self, context_mean):
        config = self.config
    ...
    def decode(self, expanded_output):
        """ Decode the expanded output of the RNN into a word. """
        config = self.config
