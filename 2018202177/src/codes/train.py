...
def train(args, train_loader, val_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, log_path):
    # checkpoint_dict = torch.load('logs/tmp/25_checkpoint.pth.tar')
    # epoch_history = checkpoint_dict['epoch']
    # epochs_since_improvement = checkpoint_dict['epochs_since_improvement']
    # encoder.load_state_dict(checkpoint_dict['encoder'])
    # decoder.load_state_dict(checkpoint_dict['decoder'])
    ...
    for epoch in range(epoch_history + 1, args.epochs):
        print(epoch)
