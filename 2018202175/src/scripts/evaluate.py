...
def score(truth_f, sub_f):
    ...
    return np.mean(mrrs), np.mean(ndcg5s), np.mean(ndcg10s)


if __name__ == '__main__':
    truth_file = open('../dataset/test/truth.json', 'r')
    submission_answer_file = open('../dataset/test/answer.json', 'r')
    output_file = open('../dataset/test/scores.txt', 'w')

    mrr, ndcg5, ndcg10 = score(truth_file, submission_answer_file)
    result = f'MRR: {mrr:.4f}\nnDCG@5: {ndcg5:.4f}\nnDCG@10: {ndcg10:.4f}'
    print(result)
