...
class Encoder(nn.Module):
    def __init__(self, encoded_image_size=14):
        ...
        # 改进1：使用预训练的 ResNet-50 并不能得到很好的结果， 所以选择使用 ResNet-101
        cnn_ext = torchvision.models.resnet101(pretrained=True)  # 使用预训练的 resnet-101
...
class AttentionModule(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden):
        ...
        att = self.relu(att)  # 用 relu 作用 att，提供非线性,激活函数
        ...
        attention_weighted_encoding = (encoder_out * alpha.unsqueeze(2)).sum(1)  # (bs, encoder_dim)，每一个元素：第bs个imge 第ed层的特征值
...
class DecoderWithAttention(nn.Module):
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size, encoder_dim=2048, dropout=0.5):
        ...
        # 定义 LSTMCell 作为 Decoder 中的序列模块，输入是 embed + encoder_out
    ...
    def forward(self, encoder_out, encoded_captions, caption_lens):
        ...
        embeddings = self.embedding(encoded_captions)  # 得到 encoded_captions 的词向量, (bs, max_caption_lens, embed_dim)

        # 初始化 LSTM hidden state 和 cell state
        h, c = self.init_hidden_state(encoder_out)

        # 我们不会对 <end> 位置进行解码，f所以解码的长度是 caption_lens - 1
